...
# Merkava120 - I copied CvInfoScreen.py and hacked it up to make this. merk.facinterface
...
class CvFactionScreen2:
	"Faction Screen: this screen displays all the factions known to the player."
...
############################################### STATISTICS ###############################################
		...
		# merk.facint
		#screen = CyGInterfaceScreen("FactionScreen", CvScreenEnums.FACTION_SCREEN)
...
#BUG: Change Graphs - start
	...
	# Merkava120 - pulled this from Platy's domestic advisor and hacked it up					
	def GeneralTable(self):
		screen = CyGInterfaceScreen("DomesticAdvisor", CvScreenEnums.DOMESTIC_ADVISOR)
		iPlayer = CyGame().getActivePlayer()
		pPlayer = gc.getPlayer(iPlayer)		
		self.nTableWidth = screen.getXResolution() - 40
		self.nTableHeight = screen.getYResolution() - 125
		...
		for fac in xrange(iFactions):
			lFactions.append(fac)
		...
		iWidth = (self.nTableWidth - 180 - 174)/(4)
		...
		screen.setTableColumnHeader("DomesticTable", 2, CyTranslator().getText("Popularity", ()), iWidth)
		screen.setTableColumnHeader("DomesticTable", 3, CyTranslator().getText("Wealth", ()), iWidth)
		screen.setTableColumnHeader("DomesticTable", 4, CyTranslator().getText("Production", ()), iWidth)
		screen.setTableColumnHeader("DomesticTable", 5, CyTranslator().getText("Faction Type", ()), iWidth)
		...
		for i in xrange(len(lFactions)):
			...
			screen.setTableText("DomesticTable", 0, iRow, CyGame().getFactionName(lFactions[i]), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
			...
			screen.setTableInt("DomesticTable", 3, iRow, self.addColor(CyGame().getFactionWealth(lFactions[i]), 1), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_RIGHT_JUSTIFY)
			screen.setTableInt("DomesticTable", 4, iRow, self.addColor(CyGame().getFactionProduction(lFactions[i]), 1), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_RIGHT_JUSTIFY)
