...
public:
	...
	DllExport CvUnit* getBestDefender(PlayerTypes eOwner, PlayerTypes eAttackingPlayer = NO_PLAYER,	// Exposed to Python
		...
		bool bTestVisible = false, int iRanged = 0) const  // <advc> Need some more params // merkava120.tc specifying ranged (int so overload works)
	{
		return getBestDefender(eOwner, eAttackingPlayer, pAttacker, bTestAtWar,
				...
				true, false, iRanged); // merkava120.tc specifying ranged
	}
	CvUnit* getBestDefender(PlayerTypes eOwner,
			...
			bool bTestCanAttack, bool bTestAny = false, int iRanged = 0) const; // </advc> // merkava120.tc specifying ranged
