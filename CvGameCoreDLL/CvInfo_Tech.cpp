...
m_bTrade(false),
m_bDisable(false),
m_bDynamicOnly(false), // merk.dtlim
m_bGoodyTech(false),
...
void CvTechInfo::read(FDataStreamBase* stream)
{
	...
	stream->Read(&m_bEventOnly); // merk.dtlim
...
void CvTechInfo::write(FDataStreamBase* stream)
{
	...
	stream->Write(m_bEventOnly); // merk.dtlim
...
bool CvTechInfo::read(CvXMLLoadUtility* pXML)
{
	...
	pXML->GetChildXmlValByName(&m_bDynamicOnly, "bEventOnly", false, false); // merk.dtlim
