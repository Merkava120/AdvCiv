...
// merkava.mb1tL
void CvMap::calculateActualLatitudes()
{
	int iTotalTopTemp = 0;
	for (int iX = 0; iX < getGridWidth(); iX++)
	{
		iTotalTopTemp += GC.getInfo(getPlot(iX, 0).getTerrainType()).getTemperature();
	}
}
// mb1d
void CvMap::addBiomeSigns()
{
	for (int i = 0; i < numPlots(); i++)
	{
		/*if (GC.getGame().getPlotBiome(i) == -1)
		{
			GC.getMap().plotByIndex(i)->setImprovementType((ImprovementTypes)0);
			continue;
		}*/
		// mm turns out signs require some fancy python thing so, let's use bonuses and remove all features for easy viewing
		if (GC.getMap().plotByIndex(i)->isWater() && GC.getDefineINT("ADD_UNITS_IN_DEBUG_MODE") > 0)
			if (GC.getGame().getPlotBiome(i) < GC.getNumUnitInfos() && GC.getGame().getPlotBiome(i) > -1)
				GET_PLAYER(BARBARIAN_PLAYER).initUnit((UnitTypes)GC.getGame().getPlotBiome(i), GC.getMap().plotByIndex(i)->getX(), GC.getMap().plotByIndex(i)->getY());
			else
				continue;
		else if (GC.getMap().plotByIndex(i)->isWater())
			continue;
		GC.getMap().plotByIndex(i)->setFeatureType(NO_FEATURE);
		GC.getMap().plotByIndex(i)->setBonusType(NO_BONUS);
		if (GC.getGame().getPlotBiome(i) < GC.getNumBonusInfos() && GC.getGame().getPlotBiome(i) > -1)
			GC.getMap().plotByIndex(i)->setBonusType((BonusTypes)GC.getGame().getPlotBiome(i));
		else if (GC.getGame().getPlotBiome(i) - GC.getNumBonusInfos() > -1 && GC.getGame().getPlotBiome(i) - GC.getNumBonusInfos() < GC.getNumImprovementInfos())
			GC.getMap().plotByIndex(i)->setImprovementType((ImprovementTypes)(GC.getGame().getPlotBiome(i) - GC.getNumBonusInfos()));
	}
}
// merkava END
